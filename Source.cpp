...
double Num(char* FileName, Student* s, int N)
{
	ifstream f(FileName, ios::binary);
	if (!f)
	{
	...
	for (int i = 0; i < N; i++)
	{
		if (s[i].physics == 5)
		{
			cout << "Студенти які отримали 5 з Фізики: " << s[i].surnamed << endl;
		}
	}
	return m;

}
double Prots(char* FileName, Student* s, int N)
{
	ifstream f(FileName, ios::binary);
	if (!f)
	{
	...
	for (int i = 0; i < N; i++)
	{
		if (s[i].math == 5 && s[i].physics == 5 && s[i].informatik == 5)
		{
			k++;
		}
	}
...
int main()
{
	...
	Num(FileName, s, amSt);
	cout << "Процент студентів, які вчаться на «відмінно»: " << Prots(FileName, s, amSt) << "%" << endl;
	return 0;
}	